...
aws s3 cp s3://jackiu-`echo $AWS_DEFAULT_REGION`/ssl.conf /etc/httpd/conf.d/ssl.conf
...
aws s3 cp s3://jackiu-`echo $AWS_DEFAULT_REGION`/poc-0.0.1-SNAPSHOT.war .

aws s3 cp s3://jackiu-`echo $AWS_DEFAULT_REGION`/rds-ca-2015-root.pem .
